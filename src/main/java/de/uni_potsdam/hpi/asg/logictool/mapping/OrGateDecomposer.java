...
public class OrGateDecomposer {
    private static final Logger logger = LogManager.getLogger();

    private Netlist             netlist;

    public OrGateDecomposer(Netlist netlist) {
        this.netlist = netlist;
    }

    public boolean decomposeOR(NetlistTerm term) {
        ...
        while(id < num) {
            ...
            if(ispos) {
                t1bdd = t1bdd.or(var.toBDD());
            } else {
                t1bdd = t1bdd.or(var.toNotBDD());
            }
            id++;
        }

        BDD t2bdd = netlist.getFac().zero();
        while(id < vars.size()) {
            NetlistVariable var = it.next();
            Boolean ispos = BDDHelper.isPos(term.getBdd(), var);
            if(ispos == null) {
            ...
            if(ispos) {
                t2bdd = t2bdd.or(var.toBDD());
            } else {
                t2bdd = t2bdd.or(var.toNotBDD());
            }
            id++;
        }
        ...
        BDD newbdd = netlist.getFac().zero();
        newbdd = newbdd.or(var1.toBDD());
        newbdd = newbdd.or(var2.toBDD());
